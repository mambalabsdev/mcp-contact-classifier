...
const TOOL_NAME = "classify_contact";
const ACTOR_ID = "0lGSeYJmniXhGANnO";
// Every input the live actor marks required must be required in the tool schema.
const ACTOR_REQUIRED = ["job_title"];
// Every input the live actor exposes must be exposed by the tool.
const ACTOR_INPUTS = ["company_domain", "full_name", "job_title", "llm_model", "llm_provider", "skipCache", "use_llm_fallback", "verify_position"];
...
test("package identity matches the locked naming convention", () => {
  ...
  assert.deepEqual(key, ["mamba-contact-classifier"]);
  assert.deepEqual(mcp.mcpServers[key[0]].args, ["-y", pkg.name]);
  assert.equal(pkg.name, "@mambalabsdev/mcp-contact-classifier");
  assert.equal(pkg.mcpName, "com.mambabuilt/mcp-contact-classifier");
});
